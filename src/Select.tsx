...
export interface SelectProps<ValueType = any, OptionType extends BaseOptionType = DefaultOptionType>
  ...
  mode?: 'combobox' | 'multiple' | 'tags' | 'mixed';
...
const Select = React.forwardRef(
  (props: SelectProps<any, DefaultOptionType>, ref: React.Ref<BaseSelectRef>) => {
    ...
    const onInternalSelect = useRefFunc<OnInternalSelect>((val, info) => {
      ...
      const fdata: FlattenOptionData<DefaultOptionType>[] = displayOptions.filter(
        (v) => v.value === val,
      );
      const { singleKey } = fdata[0];

      if (mergedSelect) {
        if (singleKey) {
          cloneValues = mergedValues.filter((v) => v.singleKey !== singleKey);
          cloneValues = multiple ? [...cloneValues, val] : [val];
        } else {
